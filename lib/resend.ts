...
export async function sendPurchaseEmail(to: string, name?: string) {
  ...
  if (buyerResult.error) throw new Error(buyerResult.error.message || 'Purchase email failed')
  ...
  if (ownerResult.error) console.error('[resend] owner purchase alert failed', ownerResult.error)
  return buyerResult.data
}
...
export async function sendLeadMagnetEmail(to: string, firstName?: string) {
  ...
  if (result.error) throw new Error(result.error.message || 'Lead magnet email failed')
  return result.data
}
